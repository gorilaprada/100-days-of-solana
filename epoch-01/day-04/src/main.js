...
function renderWalletLists(wallets) {
  ...
  statusDiv.textContent = `Found ${solanaWallets.length} wallets(s):`;
  walletListDiv.innerHTML = "";
...
async function connectWallet(wallet) {
  ...
  if (!connectFeature) {
    errorDiv.textContent = "This wallet does not support connecting."
    return;
  }
  ...
  try {
    ...
    connectedDiv.innerHTML = `
      ...
      <div class="balance">${balanceInSol}</div>
