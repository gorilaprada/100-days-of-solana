...
async function loadOrCreateWallet() {
  try {
    ...
    console.log(`Loading existing wallet: ${wallet.address}`)
    return wallet
  } catch {
    ...
    // Export the pub key (raw format works for public keys)
    const publicKeyBytes = new Uint8Array(
      await crypto.subtle.exportKey("raw", keyPair.publicKey)
    );

    // Export the private key using pkcs8 format
    // (Node js nor Deno support "raw" export for Ed25519 private keys)
